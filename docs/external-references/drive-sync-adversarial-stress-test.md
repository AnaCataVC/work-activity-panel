...
>
> **Implementation Status (2026-09-06):** With the scan/hash fast-path confirmed healthy in practice, the remaining slowness traced to the Apps Script side of vulnerability #2: every `doPost` re-walked the destination path from the root with `getFoldersByName` on every single upload, with no memory across calls. `Code.gs` now caches resolved folder IDs in `PropertiesService`, turning a repeat upload into an already-known folder into a direct `getFolderById` lookup instead of a fresh search per path segment. Self-heals if a cached folder is deleted/moved, and also treats a cached folder sitting in Drive's Trash as a miss (`getFolderById` alone does not throw for trashed folders, which would otherwise silently accumulate uploads until Drive's auto-purge). See [drive-sync-appsscript-folder-id-cache.md](../learning/drive-sync-appsscript-folder-id-cache.md). The `LockService`/`SemaphoreSlim(1,1)` invariant itself is untouched.
>
> **Implementation Status (2026-09-10):** Uploads still stayed slow whenever many files actually changed, since each one was still its own sequential `doPost` call — no cache helps a file that genuinely needs uploading. `DriveSyncService` now batches up to 8 files (capped at 9 MB raw / ~12 MB base64-encoded, safely under the ~25-30 MB ceiling documented in vulnerability #3 above) into a single JSON POST, and `Code.gs` loops over them under one `LockService` acquisition instead of one per file. This is a breaking wire-protocol change (JSON body instead of form fields) — the deployed script must be updated together with the app. See [drive-sync-request-batching.md](../learning/drive-sync-request-batching.md).
