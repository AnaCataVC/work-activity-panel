...
- `DriveSyncService.ClassifyFile` is the single source of truth for the per-file skip/upload decision (fast-path metadata check → SHA-256 fallback) and never writes to the hash cache itself — `RunSyncAsync` decides whether to persist a `MetadataStale` upgrade. `PreviewOutOfSyncAsync` reuses the same classifier in read-only mode (always with `forceFullSync: false`) to answer "what would a sync upload" without uploading or touching `sync_hashes.json` — this backs the "Ver qué está desincronizado" menu item next to "Verificar y forzar sincronización total" in the Drive card. Keep both call sites on `ClassifyFile` instead of re-implementing the decision inline.
- The `Code.gs` bridge template (`docs/google-setup-guide.md`) caches resolved Drive folder IDs in `PropertiesService` so a repeat upload into an already-seen path does a direct `getFolderById` lookup instead of re-walking the tree with `getFoldersByName` on every call — this defends against sync staying slow once most of a tree is already mirrored. Keep this cache when editing the template; see `docs/learning/drive-sync-appsscript-folder-id-cache.md`.
- Uploads are batched: `DriveSyncService.BuildBatches` groups pending files (cap: `MaxBatchFileCount` files or `MaxBatchRawBytes` raw bytes, whichever comes first) into a single JSON POST per batch instead of one POST per file, so the fixed per-call cost of Apps Script (cold start + `LockService` acquisition) is paid once per batch — this is the main defense against sync staying slow when many files changed since the last run. The wire format is always `{ authToken, files: [...] }` (a batch of one for a single-file upload, retry, or connection test), matched in `Code.gs`'s `doPost` by reading `e.postData.contents` instead of `e.parameter`. The byte cap keeps the base64-encoded payload under Google's undocumented ~25-30 MB HTTP body ceiling for Apps Script Web Apps. Keep the app and the deployed `Code.gs` on the same protocol version — changing one without the other breaks every upload; see `docs/learning/drive-sync-request-batching.md`.
